...
parse() {
	...
	sfdp -x -Goverlap=scale -Tsvg ./output/parser/out_parser_"$1".dot -o ./output/parser/out_graph_"$1".svg
}
...
final() {
	[ -d "output/final" ] || mkdir -p output/final
	[ -f "src/parsetab.py" ] && rm src/parsetab.py
	python ./src/parser.py -input ./tests/final/test_final_"$1".c
	mv ./mips_generated.s ./output/final/mips_"$1".s
}

usage="./testing_script.sh <lex|parse|semantic|final> <test_number>"
