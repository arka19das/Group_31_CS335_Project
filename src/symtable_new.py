...
class SymbolTable:
    ...
    def translate_type(self, typename: str) -> Union[None, str]:
        ...
        return (
            ...
            if self.parent is not None and tname is None
            else tname
        )
    ...
    def lookup_type(self, typename: str) -> Union[dict, None]:
        ...
        return (
            self.parent.lookup_type(typename)
            if self.parent is not None and t is None
            else t
        )
    ...
    def display(self) -> None:
        ...
        print("-" * 100)
        print(
            f"SYMBOL TABLE: {self.table_name}, TABLE NUMBER: {self.table_number}, FUNCTION SCOPE: {self.func_scope}"
        )
        print("-" * 51)
        ...
        with open("symtables.csv", mode="a+") as sym_file:
            sym_writer = csv.writer(
                sym_file, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
            )
            if num_display_invocations == 0:
                sym_writer.writerow(
                    [
...
def compute_offset_size(
    dsize: int, is_array: bool, dimensions: List[int], entry, typeentry
) -> int:
    if not is_array:
        return dsize
    else:
...
def compute_storage_size(entry, typeentry) -> int:
    ...
    if _c > 0:
        ...
        return compute_storage_size(
            {"type": t, "pointer_lvl": _c}, get_current_symtab().lookup_type(t)
        )
    global DATATYPE2SIZE
...
def get_tmp_closure(rettype: str, argtypes: list = []) -> str:
    ...
    symTab = get_current_symtab()
    symTab.insert(
        {"name": vname, "return type": rettype, "parameter types": argtypes}, kind=1
    )
    return vname
